            ...
            for event in pygame.event.get():
                ...
                if coords:
                    ...
                    if self.game_cont.make_move(coords):
                        ...
                        self.textbox_error.set_visible(False)
                        pygame.mixer.Sound.play(pygame.mixer.Sound(self.game_cont.base_dir / "assets" / "successful_move_sound.mp3")).set_volume(0.5)
                    # Handling invalid move with error message and sound.
                    else:
                        self.textbox_error.set_text(self.game_cont.get_fail())
                        self.textbox_error.set_visible(True)
                        pygame.mixer.Sound.play(pygame.mixer.Sound(self.game_cont.base_dir / "assets" / "invalid_move_sound.mp3")).set_volume(0.5)
                ...
                if self.button_save.handle_event(event) == 'clicked':
                    if self.inputbox_username.value == "": 
                        ...
                        self.game_cont.save_game(self.game_cont.base_dir / "saves" / f"{self.inputbox_username.value}_level2_save.json")    
